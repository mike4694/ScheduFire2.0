...
import model.dao.CapoSquadraDao;
...
public class LoginServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if(session.getAttribute("ruolo")!=null) {
			...
			if(ruolo.equalsIgnoreCase("caposquadra")) {
				
				response.sendRedirect("HomeCSServlet");
				return;
			}
			...
			if (username == null || username.equals(""))
				...
				if (utente == null) {
					...
					if (passwordBase256format.equals(utente.getPassword())) {
						...
						if (utente.getRuolo().equalsIgnoreCase("caposquadra")) {
							CapoSquadraBean caposquadra=CapoSquadraDao.ottieni(username);
							session.setMaxInactiveInterval(2700); //45 min
							session.setAttribute("caposquadra", caposquadra);
							session.setAttribute("notifiche", new Notifiche());
							response.sendRedirect("HomeCSServlet");
							return;
							//RequestDispatcher dispatcher = request.getRequestDispatcher("WebContent\\JSP\\LoginJSP.jsp");
